% Region-wise quantitative analysis within one brain

%% Regional quantitation 

dataset_dir = 'X:\Ryan\Light_Sheet_Imaged_Brains\Comparison\11-07-2024_OC3_T10_TAIL';
...
% Instead of looping over multiple brains, pick one brain
data_dir = data_subdirs{1};
[volumes, counts, densities, df_cells] = quantify(data_dir, classes, header, save_table, dataset_dir);

save(fullfile(result_dir,'results.mat'), 'volumes','counts','densities','df_cells');

%% Statistics between regions
% Removed group assignment and between-brain statistical analysis

% Simply save the quantified results (df_cells) as the region statistics
%save(fullfile(result_dir,'stats.mat'), 'df_cells');
%if save_table
%    writetable(df_cells, fullfile(result_dir,'stats.csv'))
%end

%% Functions

function [df_volumes, num, density, df_cells] = quantify(data_dir, classes, header, save_table, dataset_dir)
    ...
    mhd_annotation_file = fullfile(dataset_dir, 'volume\result.mhd');
    ...
    for i = 1:length(classes)
        df_new = zeros(size(df_volumes));
        % Replace readtable with textscan for parsing df_csv
        csv_file = fullfile(dataset_dir, 'cell_registration', num2str(i), 'cell_registration.csv');
        fid = fopen(csv_file, 'r');
        if fid == -1
            error('Cannot open file: %s', csv_file);
        end
        C = textscan(fid, repmat('%s', 1, length(header)), 'Delimiter', ',', 'ReturnOnError', false);
        fclose(fid);
        dataCell = [C{:}]; % NxM cell array
        df_csv = cell2table(dataCell, 'VariableNames', header(1:size(dataCell,2)));
        % ...existing code...
        if iscell(df_csv.id)
            df_csv_id = str2double(df_csv.id);
        else
            df_csv_id = df_csv.id;
        end
        % Preallocate count with length matching the structure ids from the template
        count = zeros(1, length(ids));
        counted = histcounts(df_csv_id, 'BinMethod', 'integers');
        % Ensure we only assign within the bounds of count
        n = min(length(counted)-1, length(ids)-1);
        count(2:n+1) = counted(2:n+1);
        count(1) = 0;
        for j = 2:length(ids)
