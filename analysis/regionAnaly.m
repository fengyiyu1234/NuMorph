...
dataset_dir = 'X:\Ryan\11-07-2024\oc3-t10-tail-p5\numorph';
...
for i = 1:length(data_subdirs)
    ...
    [volumes{1,i},counts{1,i},densities{1,i},...
        df_results{1,i}] = quantify(data_dir,classes, header, save_table,dataset_dir);
end
...
function [df_volumes, num, density, df_cells] = quantify(data_dir, classes, header, save_table, dataset_dir)
    ...
    for i = 1:length(classes)
        df_new = zeros(size(df_volumes));
        df_csv = readtable(fullfile(dataset_dir,'cell_registration',num2str(i),'cell_registration.csv'),'ReadVariableNames',false);
        %df_csv = paddata(df_csv,length(header),Dimension=2); % matlab 2023b
        df_csv.Properties.VariableNames = header(1:length(df_csv.Properties.VariableNames));
