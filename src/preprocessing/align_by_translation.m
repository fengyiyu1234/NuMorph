...
for i = 1:size(order_m,1) % i is the image number
    % Get z position 
    z_idx = order_m(i,1);
        ...
        if order_m(i,j) > min_signal
           ...
           if i > length(z)*0.1
               ...
               if d > shift_threshold
                   ...
                   if isempty(m_subset)
                       %do nothing
                       disp("translation is too far from median " + ...
                           "and it could not take the nearest translation" + ...
                           " (align_by_translation.m:line 301)")
                   else
                       [~,nearest_idx] = min(abs(m_subset(:,1)-z_idx));
                       tform.T(3) = m_subset(nearest_idx,idx+1);
                       tform.T(6) = m_subset(nearest_idx,idx+2);
